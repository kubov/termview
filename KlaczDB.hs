...
getTermsQuery = "select _name, count(*) from _entry join _term on (_term._oid = _entry._term_oid) where _term._visible = 't' and _entry._visible = 't' group by _name order by upper(_name);"
...
getMembersQuery = "select _added_by, count(*) as cc from _entry join _term on (_term._oid = _entry._term_oid) where _term._visible = 't' and _entry._visible = 't' group by _added_by having count(*) > 0 order by upper(_added_by);"
